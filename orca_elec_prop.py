...
def dipole_moment(infile):
    ...
    -------
    dip array: tuple.
        Returns a tuple of arrays containing electronic, nuclear and total dipole moment values.
    '''
    with open(log) as f:
      for line in f:
...
def last_coord(infile):
    ...
    """
    data=orca_reader(infile)
    coords=np.array(data.metadata['coords'])
    natoms=data.natom
...
def qm_charges(infile, method="lowdin"):
    ...
    charges_total[str(method)]=charges[str(method)]
...
def mo_energies(infile):
    ...
    mo_energies["mo_energies"]=np.array(list_mo_energies(log)[0:idx_homo])
...
if __name__ == "__main__":
  ...
  # Dictionaries for singlets and triplets and
  #function to merge these dictionaries.

  #sin,trip=energy_levels(log)
  #final = dict(chain(sin.items(), trip.items()))

  # Polarization matrices from inputfile (WORKING)
  #elec, nuc, tot=quad_polarization(log)

  #Function to read Dipole moment and its components:
  #elec, nuc, tot=dipole_moment(log)

  #Function to display QM charges using the Lowdin method. This can be also
  #changed to mulliken
  #print (qm_charges(log,"lowdin"))
